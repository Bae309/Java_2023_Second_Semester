...
/*
 * GUI 클래스에서 이벤트를 처리하는 방법(독립 클래스 버전)
 */
...
class Lc implements ActionListener {
	...
	public Lc (JLabel jl, JButton jb) {
		this.jl = jl;
		this.jb = jb;
	}
...
class Event1_2 extends JFrame {
	public JLabel jl; // JLabel 객체를 속성으로 등록
	
	public Event1_2() { // 생성자 생성
		// 컨테이너 생성
		...
		Lc lc = new Lc(jl, jb);
		jb.addActionListener(lc); // 버튼 객체에 리스너 등록
		
		setTitle("Event Test1_2");
		setSize(300,200);
